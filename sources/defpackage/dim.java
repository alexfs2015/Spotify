package defpackage;

import android.app.Activity;
import android.content.Context;
import android.text.TextUtils;
import android.util.DisplayMetrics;
import android.view.MotionEvent;
import android.view.View;
import com.google.android.gms.internal.ads.zzcw;
import java.lang.reflect.InvocationTargetException;
import java.lang.reflect.Method;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;
import java.util.concurrent.Callable;
import java.util.concurrent.ExecutorService;
import java.util.concurrent.TimeUnit;

/* renamed from: dim reason: default package */
public class dim extends dil {
    private static final Object s = new Object();
    private static boolean t = false;
    private static long u = 0;
    protected boolean r = false;
    private String v;
    private boolean w = false;
    private boolean x = false;
    private djm y;

    static {
        dim.class.getSimpleName();
    }

    protected dim(Context context, String str, boolean z) {
        super(context);
        this.v = str;
        this.r = z;
    }

    private static djj a(djc djc, MotionEvent motionEvent, DisplayMetrics displayMetrics) {
        Method a = djc.a("ryulhTIO44CH2QVINEPOtPkpQOSdbbq7HgisLeHH33VCs+TjwYWzYttvzJcQiI1k", "dnKLchXhJgI3U7L4th0PQJWe3DOIJJ7MUji5p7t5mjc=");
        if (a == null || motionEvent == null) {
            throw new zzcw();
        }
        try {
            return new djj((String) a.invoke(null, new Object[]{motionEvent, displayMetrics}));
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new zzcw(e);
        }
    }

    protected static synchronized void a(Context context, boolean z) {
        synchronized (dim.class) {
            if (!t) {
                u = System.currentTimeMillis() / 1000;
                a = b(context, z);
                t = true;
            }
        }
    }

    private static void a(List<Callable<Void>> list) {
        if (a != null) {
            ExecutorService executorService = a.b;
            if (executorService != null && !list.isEmpty()) {
                try {
                    executorService.invokeAll(list, ((Long) dqe.f().a(dtg.bB)).longValue(), TimeUnit.MILLISECONDS);
                } catch (InterruptedException e) {
                    String.format("class methods got exception: %s", new Object[]{djk.a((Throwable) e)});
                }
            }
        }
    }

    private static djc b(Context context, boolean z) {
        if (a == null) {
            synchronized (s) {
                if (a == null) {
                    djc a = djc.a(context, "fFhi0cTZpyVQYwMwl7BCfa0fa6esmkRUaNr4ktfJYZ8=", "", z);
                    if (a.k) {
                        if (((Boolean) dqe.f().a(dtg.bH)).booleanValue()) {
                            a.a("4o7tecxtkw7XaNt5hPj+0H1LvOi0SgxCIJTY9VcbazM/HSl/sFlxBFwnc8glnvoB", "RgSY6YxU2k1vLXOV3vapBnQwJDzYDlmX50wbm2tDcnw=", new Class[0]);
                        }
                        a.a("1QeH3Cf7T53ayw17Ebbo9YTdhU+IFx0X5nCtC5gZQym4uicOVPXxYWmMK9k58i8n", "bHJRpFJ+2R5LAbYQUBDMyfYpLd1oiGixlpIqMJOBQPY=", Context.class);
                        a.a("1DzTSPdcTVkbB0cTQEaPo/MU1E1TGypt38nYJILt/8CcQbOuYjF6hLShUHnLb5uw", "dkCo8xze5sFyp7Msrb8F1JKjVIP7LutspgZPYQvsMlo=", Context.class);
                        a.a("+8WN7RfVBrTKK5NXM+y9JA4Ygu2jPZokHNFGgulhBRZ32uCnyKWWGQNTE1weoQCM", "peILYfFejTn8xmazfpiZWeK2eaYjjxEBmAZ8bUD9PWU=", Context.class);
                        a.a("6hrke/fCJ5tLCck7L6DvYxDB/To2PLUVGD3lbM1fdL2y9IScTypVRvrPFLLn4rDM", "+GAY853+DQRMYkeMG8Hbv1THvRgwf43Jh+3p4yQDqGw=", Context.class);
                        a.a("f8J+j+UhiTEjRF3iyDcFnM9Rm6LC/P+2PE2qff5S0gHSDPzrH4HNcnCaI+twvhbv", "MOBosqV6pejtL/WEgsHP3Y2p2WtApcEEa2XyqqRQCo0=", Context.class);
                        a.a("bYi99wWoqjOnUF4nnSWQsu9qLqARRsoauNez1uhJPws2YAlolLPh/xgKDnyI06MJ", "pjeIE8EsMfMk4SIebXHXNWKXTpK8v5vG6YqOxp1QH1M=", Context.class);
                        a.a("ug+duzFNTlMWhYez8qKOR3bxkM8vLOPy9xVUBCCMTrplDFf/BnF2bNmA7hw0iYtI", "ogMtakENM6NAXJL7DCVTIbgawd5T3ZVhEaXugMAz+D4=", Context.class);
                        a.a("PRnAd1qZLf3/u2x/ZslcDq2vQzDbmFHkH/jSij536nFGwYo+g6o8PpQOJcDeuz++", "99eM4BYfXxOCBAhS2wQJENWQ4BMWrR/wKqrfHx1Ws8w=", Context.class);
                        a.a("ryulhTIO44CH2QVINEPOtPkpQOSdbbq7HgisLeHH33VCs+TjwYWzYttvzJcQiI1k", "dnKLchXhJgI3U7L4th0PQJWe3DOIJJ7MUji5p7t5mjc=", MotionEvent.class, DisplayMetrics.class);
                        a.a("oT49Nu6m3oNdAQwbQzvwr/RE4TwZbR8mJKypakrI/GqBgyD2niyVic8N9UaqvU10", "LS3Cw7DlWSqpRsXrajoVsCxXOedCBnQXFZcLjXzD8so=", MotionEvent.class, DisplayMetrics.class);
                        a.a("1GzFbNgtZBsZ6xBrCbOvmw4GuyabWdtXgJqf+Aoik3E00oSTfDga4UiXvbD1YLYB", "1EiWqNFog84woyMiTveFonfDO5rsRz44QrKedC4xw+8=", new Class[0]);
                        a.a("R1L5/7J00Fko/kDi6VDsBhCzpp4/7wzabnzdihUS7rPLvaXLFWrERChyvYYgVJRF", "4WtEOgM4wQ7wiw1OB3pMyAbaplBizcjE6LRBT/nrDgM=", new Class[0]);
                        a.a("uFEr0moMuWuurgrUpfSBjyUksFv6IqKMSuNqo2jCjRmu9JHDaSBf9LDKs2u/XE/B", "hDtufeJTxJSs5qQMCIPuyzc9ZPOtO/34yyf8Kf44rGo=", new Class[0]);
                        a.a("C4t4eP72HKoZsX62gBXGicLgvQbyKikYhXkvESjpMVAOEyUibmrLmpkJ5lZygzZE", "onbJNRechKgg7ilJYzLDEQ07M0uNlDZp05nZ4h6WJ40=", new Class[0]);
                        a.a("YOE4tLd8IhAbsNGoUEFHfu51/79yG0YlK4bMxi/e6gOIrtpqPN+TQgqpfYNBoiRy", "aFY444Zz3/sMYl+Q7uujofvlLWJ37651UMbaKJCFBnU=", new Class[0]);
                        a.a("Q7gHsedaZvBKvqRLQGMIPKihViSMmXus6bBgU0lNdv+Pm9sSlP+Z+JltWG56lF1K", "mrdNgIRzsAu+cEIamFRkeCivuYIijUjvD76p38JyQec=", new Class[0]);
                        a.a("LJD7GILw6ox/o3kYqhR5FF4FDylErrAaJ66Y8V8gtnNGF8DcetGJrYvTlHeK8kyR", "GKdCM/OywOIR4FMFS5XrDdTIpVvhoMFma/KSfRUdsbQ=", Context.class, Boolean.TYPE, Boolean.TYPE);
                        a.a("nXKe8Ev09tosW935mj67BeJvnsHiQzgrotB4vyuZm+aJQgx77SXcf757aVGtnDOm", "yl+F7AaTCLci2yEfPOMTGPVVFLoqw9fKWSf4HTpN46M=", StackTraceElement[].class);
                        a.a("j5/WVRJsgiWNVIg0a6aXYQYocHm4rEEozf0LrIkPezct93ISwgyVBTLRIkEvztpn", "gCYklRkrI0pyKJ3j1u5Yl6Jj12a6PD1K39diuw7+f7o=", View.class, DisplayMetrics.class);
                        a.a("WiMYovSpRj6RF1r6zXON0hYVyLxgllUAYyMyyeBevdbJbdYcoBpd8nDjCwwclcSu", "vpq7z91A+vN3MIS65A+GwiFRgEFOFpQOdK5Z+mjW21w=", Context.class, Boolean.TYPE);
                        a.a("wxabFp8V9bGKut15duzoi2M8ew0t/HWGFw3NoPp8pFanKwQpdIx0WzpFbmJaLX9T", "grSrEy+Ml8J7boJNrN9uNiXpddsqwR6A9NVesV/hHT8=", View.class, Activity.class, Boolean.TYPE);
                        a.a("aH422z6bUtRL57gOABo0AU5IQDHmWvjsQgQZk64bzhjXVYUZmajAsUz/rrnMYVsw", "UAmRxgiWCEqHprf42V+e65VvIvUfFXeXbaMy5B8vzwA=", Long.TYPE);
                    }
                    a = a;
                }
            }
        }
        return a;
    }

    /* access modifiers changed from: protected */
    public final long a(StackTraceElement[] stackTraceElementArr) {
        Method a = a.a("nXKe8Ev09tosW935mj67BeJvnsHiQzgrotB4vyuZm+aJQgx77SXcf757aVGtnDOm", "yl+F7AaTCLci2yEfPOMTGPVVFLoqw9fKWSf4HTpN46M=");
        if (a == null || stackTraceElementArr == null) {
            throw new zzcw();
        }
        try {
            return new dja((String) a.invoke(null, new Object[]{stackTraceElementArr})).a.longValue();
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new zzcw(e);
        }
    }

    /* access modifiers changed from: protected */
    public final dci a(Context context, View view, Activity activity) {
        List list;
        Long l;
        dci dci = new dci();
        if (!TextUtils.isEmpty(this.v)) {
            dci.b = this.v;
        }
        djc b = b(context, this.r);
        if (!b.k) {
            dci.k = Long.valueOf(16384);
            list = Arrays.asList(new Callable[]{new djx(b, dci)});
        } else {
            try {
                djj a = a(b, this.b, this.q);
                dci.g = a.a;
                dci.h = a.b;
                dci.i = a.c;
                if (this.p) {
                    dci.u = a.d;
                    dci.v = a.e;
                }
            } catch (zzcw unused) {
            }
            ddc ddc = new ddc();
            if (this.d > 0 && djk.a(this.q)) {
                ddc.m = Long.valueOf(djk.a(this.k, this.q));
                ddc.n = Long.valueOf(djk.a((double) (this.n - this.l), this.q));
                ddc.o = Long.valueOf(djk.a((double) (this.o - this.m), this.q));
                ddc.r = Long.valueOf(djk.a((double) this.l, this.q));
                ddc.s = Long.valueOf(djk.a((double) this.m, this.q));
                if (this.p && this.b != null) {
                    long a2 = djk.a((double) (((this.l - this.n) + this.b.getRawX()) - this.b.getX()), this.q);
                    if (a2 != 0) {
                        ddc.p = Long.valueOf(a2);
                    }
                    long a3 = djk.a((double) (((this.m - this.o) + this.b.getRawY()) - this.b.getY()), this.q);
                    if (a3 != 0) {
                        ddc.q = Long.valueOf(a3);
                    }
                }
            }
            try {
                djj b2 = b(this.b);
                ddc.a = b2.a;
                ddc.b = b2.b;
                ddc.h = b2.c;
                if (this.p) {
                    ddc.c = b2.e;
                    ddc.e = b2.d;
                    ddc.g = Integer.valueOf(b2.f.longValue() != 0 ? 1 : 0);
                    if (this.e > 0) {
                        if (djk.a(this.q)) {
                            double d = (double) this.j;
                            double d2 = (double) this.e;
                            Double.isNaN(d);
                            Double.isNaN(d2);
                            l = Long.valueOf(Math.round(d / d2));
                        } else {
                            l = null;
                        }
                        ddc.d = l;
                        double d3 = (double) this.i;
                        double d4 = (double) this.e;
                        Double.isNaN(d3);
                        Double.isNaN(d4);
                        ddc.f = Long.valueOf(Math.round(d3 / d4));
                    }
                    ddc.j = b2.i;
                    ddc.i = b2.j;
                    ddc.k = Integer.valueOf(b2.k.longValue() != 0 ? 1 : 0);
                }
            } catch (zzcw unused2) {
            }
            if (this.h > 0) {
                ddc.l = Long.valueOf(this.h);
            }
            dci.K = ddc;
            if (this.d > 0) {
                dci.y = Long.valueOf(this.d);
            }
            if (this.e > 0) {
                dci.x = Long.valueOf(this.e);
            }
            if (this.f > 0) {
                dci.w = Long.valueOf(this.f);
            }
            if (this.g > 0) {
                dci.z = Long.valueOf(this.g);
            }
            try {
                int size = this.c.size() - 1;
                if (size > 0) {
                    dci.L = new ddc[size];
                    for (int i = 0; i < size; i++) {
                        djj a4 = a(a, (MotionEvent) this.c.get(i), this.q);
                        ddc ddc2 = new ddc();
                        ddc2.a = a4.a;
                        ddc2.b = a4.b;
                        dci.L[i] = ddc2;
                    }
                }
            } catch (zzcw unused3) {
                dci.L = null;
            }
            List arrayList = new ArrayList();
            if (b.b != null) {
                int c = b.c();
                arrayList.add(new djx(b, dci));
                djc djc = b;
                dci dci2 = dci;
                dka dka = new dka(djc, "uFEr0moMuWuurgrUpfSBjyUksFv6IqKMSuNqo2jCjRmu9JHDaSBf9LDKs2u/XE/B", "hDtufeJTxJSs5qQMCIPuyzc9ZPOtO/34yyf8Kf44rGo=", dci2, c);
                arrayList.add(dka);
                djv djv = new djv(djc, "1GzFbNgtZBsZ6xBrCbOvmw4GuyabWdtXgJqf+Aoik3E00oSTfDga4UiXvbD1YLYB", "1EiWqNFog84woyMiTveFonfDO5rsRz44QrKedC4xw+8=", dci2, u, c);
                arrayList.add(djv);
                int i2 = c;
                dju dju = new dju(djc, "C4t4eP72HKoZsX62gBXGicLgvQbyKikYhXkvESjpMVAOEyUibmrLmpkJ5lZygzZE", "onbJNRechKgg7ilJYzLDEQ07M0uNlDZp05nZ4h6WJ40=", dci2, i2);
                arrayList.add(dju);
                djz djz = new djz(djc, "f8J+j+UhiTEjRF3iyDcFnM9Rm6LC/P+2PE2qff5S0gHSDPzrH4HNcnCaI+twvhbv", "MOBosqV6pejtL/WEgsHP3Y2p2WtApcEEa2XyqqRQCo0=", dci2, i2);
                arrayList.add(djz);
                dkb dkb = new dkb(djc, "bYi99wWoqjOnUF4nnSWQsu9qLqARRsoauNez1uhJPws2YAlolLPh/xgKDnyI06MJ", "pjeIE8EsMfMk4SIebXHXNWKXTpK8v5vG6YqOxp1QH1M=", dci2, i2);
                arrayList.add(dkb);
                djy djy = new djy(djc, "YOE4tLd8IhAbsNGoUEFHfu51/79yG0YlK4bMxi/e6gOIrtpqPN+TQgqpfYNBoiRy", "aFY444Zz3/sMYl+Q7uujofvlLWJ37651UMbaKJCFBnU=", dci2, i2);
                arrayList.add(djy);
                djt djt = new djt(djc, "6hrke/fCJ5tLCck7L6DvYxDB/To2PLUVGD3lbM1fdL2y9IScTypVRvrPFLLn4rDM", "+GAY853+DQRMYkeMG8Hbv1THvRgwf43Jh+3p4yQDqGw=", dci2, i2);
                arrayList.add(djt);
                dkj dkj = new dkj(djc, "ug+duzFNTlMWhYez8qKOR3bxkM8vLOPy9xVUBCCMTrplDFf/BnF2bNmA7hw0iYtI", "ogMtakENM6NAXJL7DCVTIbgawd5T3ZVhEaXugMAz+D4=", dci2, i2);
                arrayList.add(dkj);
                djq djq = new djq(djc, "PRnAd1qZLf3/u2x/ZslcDq2vQzDbmFHkH/jSij536nFGwYo+g6o8PpQOJcDeuz++", "99eM4BYfXxOCBAhS2wQJENWQ4BMWrR/wKqrfHx1Ws8w=", dci2, i2);
                arrayList.add(djq);
                dkg dkg = new dkg(djc, "Q7gHsedaZvBKvqRLQGMIPKihViSMmXus6bBgU0lNdv+Pm9sSlP+Z+JltWG56lF1K", "mrdNgIRzsAu+cEIamFRkeCivuYIijUjvD76p38JyQec=", dci2, i2);
                arrayList.add(dkg);
                dkf dkf = new dkf(djc, "nXKe8Ev09tosW935mj67BeJvnsHiQzgrotB4vyuZm+aJQgx77SXcf757aVGtnDOm", "yl+F7AaTCLci2yEfPOMTGPVVFLoqw9fKWSf4HTpN46M=", dci2, i2, new Throwable().getStackTrace());
                arrayList.add(dkf);
                dkk dkk = new dkk(djc, "j5/WVRJsgiWNVIg0a6aXYQYocHm4rEEozf0LrIkPezct93ISwgyVBTLRIkEvztpn", "gCYklRkrI0pyKJ3j1u5Yl6Jj12a6PD1K39diuw7+f7o=", dci2, i2, view);
                arrayList.add(dkk);
                dkd dkd = new dkd(djc, "WiMYovSpRj6RF1r6zXON0hYVyLxgllUAYyMyyeBevdbJbdYcoBpd8nDjCwwclcSu", "vpq7z91A+vN3MIS65A+GwiFRgEFOFpQOdK5Z+mjW21w=", dci2, i2);
                arrayList.add(dkd);
                if (((Boolean) dqe.f().a(dtg.bC)).booleanValue()) {
                    djp djp = new djp(b, "wxabFp8V9bGKut15duzoi2M8ew0t/HWGFw3NoPp8pFanKwQpdIx0WzpFbmJaLX9T", "grSrEy+Ml8J7boJNrN9uNiXpddsqwR6A9NVesV/hHT8=", dci, c, view, activity);
                    arrayList.add(djp);
                }
                if (((Boolean) dqe.f().a(dtg.bE)).booleanValue()) {
                    dkh dkh = new dkh(b, "aH422z6bUtRL57gOABo0AU5IQDHmWvjsQgQZk64bzhjXVYUZmajAsUz/rrnMYVsw", "UAmRxgiWCEqHprf42V+e65VvIvUfFXeXbaMy5B8vzwA=", dci, c, this.y);
                    arrayList.add(dkh);
                }
            }
            list = arrayList;
        }
        a(list);
        return dci;
    }

    /* access modifiers changed from: protected */
    public List<Callable<Void>> a(djc djc, dci dci, czk czk) {
        int c = djc.c();
        ArrayList arrayList = new ArrayList();
        if (!djc.k) {
            dci.k = Long.valueOf(16384);
            return arrayList;
        }
        djc djc2 = djc;
        dci dci2 = dci;
        djs djs = new djs(djc2, "LJD7GILw6ox/o3kYqhR5FF4FDylErrAaJ66Y8V8gtnNGF8DcetGJrYvTlHeK8kyR", "GKdCM/OywOIR4FMFS5XrDdTIpVvhoMFma/KSfRUdsbQ=", dci2, c, czk);
        arrayList.add(djs);
        djv djv = new djv(djc2, "1GzFbNgtZBsZ6xBrCbOvmw4GuyabWdtXgJqf+Aoik3E00oSTfDga4UiXvbD1YLYB", "1EiWqNFog84woyMiTveFonfDO5rsRz44QrKedC4xw+8=", dci2, u, c);
        arrayList.add(djv);
        int i = c;
        dka dka = new dka(djc2, "uFEr0moMuWuurgrUpfSBjyUksFv6IqKMSuNqo2jCjRmu9JHDaSBf9LDKs2u/XE/B", "hDtufeJTxJSs5qQMCIPuyzc9ZPOtO/34yyf8Kf44rGo=", dci2, i);
        arrayList.add(dka);
        dkc dkc = new dkc(djc2, "+8WN7RfVBrTKK5NXM+y9JA4Ygu2jPZokHNFGgulhBRZ32uCnyKWWGQNTE1weoQCM", "peILYfFejTn8xmazfpiZWeK2eaYjjxEBmAZ8bUD9PWU=", dci2, i);
        arrayList.add(dkc);
        dki dki = new dki(djc2, "R1L5/7J00Fko/kDi6VDsBhCzpp4/7wzabnzdihUS7rPLvaXLFWrERChyvYYgVJRF", "4WtEOgM4wQ7wiw1OB3pMyAbaplBizcjE6LRBT/nrDgM=", dci2, i);
        arrayList.add(dki);
        djr djr = new djr(djc2, "1DzTSPdcTVkbB0cTQEaPo/MU1E1TGypt38nYJILt/8CcQbOuYjF6hLShUHnLb5uw", "dkCo8xze5sFyp7Msrb8F1JKjVIP7LutspgZPYQvsMlo=", dci2, i);
        arrayList.add(djr);
        djt djt = new djt(djc2, "6hrke/fCJ5tLCck7L6DvYxDB/To2PLUVGD3lbM1fdL2y9IScTypVRvrPFLLn4rDM", "+GAY853+DQRMYkeMG8Hbv1THvRgwf43Jh+3p4yQDqGw=", dci2, i);
        arrayList.add(djt);
        djz djz = new djz(djc2, "f8J+j+UhiTEjRF3iyDcFnM9Rm6LC/P+2PE2qff5S0gHSDPzrH4HNcnCaI+twvhbv", "MOBosqV6pejtL/WEgsHP3Y2p2WtApcEEa2XyqqRQCo0=", dci2, i);
        arrayList.add(djz);
        dkb dkb = new dkb(djc2, "bYi99wWoqjOnUF4nnSWQsu9qLqARRsoauNez1uhJPws2YAlolLPh/xgKDnyI06MJ", "pjeIE8EsMfMk4SIebXHXNWKXTpK8v5vG6YqOxp1QH1M=", dci2, i);
        arrayList.add(dkb);
        dju dju = new dju(djc2, "C4t4eP72HKoZsX62gBXGicLgvQbyKikYhXkvESjpMVAOEyUibmrLmpkJ5lZygzZE", "onbJNRechKgg7ilJYzLDEQ07M0uNlDZp05nZ4h6WJ40=", dci2, i);
        arrayList.add(dju);
        djy djy = new djy(djc2, "YOE4tLd8IhAbsNGoUEFHfu51/79yG0YlK4bMxi/e6gOIrtpqPN+TQgqpfYNBoiRy", "aFY444Zz3/sMYl+Q7uujofvlLWJ37651UMbaKJCFBnU=", dci2, i);
        arrayList.add(djy);
        dkj dkj = new dkj(djc2, "ug+duzFNTlMWhYez8qKOR3bxkM8vLOPy9xVUBCCMTrplDFf/BnF2bNmA7hw0iYtI", "ogMtakENM6NAXJL7DCVTIbgawd5T3ZVhEaXugMAz+D4=", dci2, i);
        arrayList.add(dkj);
        djq djq = new djq(djc2, "PRnAd1qZLf3/u2x/ZslcDq2vQzDbmFHkH/jSij536nFGwYo+g6o8PpQOJcDeuz++", "99eM4BYfXxOCBAhS2wQJENWQ4BMWrR/wKqrfHx1Ws8w=", dci2, i);
        arrayList.add(djq);
        dkg dkg = new dkg(djc2, "Q7gHsedaZvBKvqRLQGMIPKihViSMmXus6bBgU0lNdv+Pm9sSlP+Z+JltWG56lF1K", "mrdNgIRzsAu+cEIamFRkeCivuYIijUjvD76p38JyQec=", dci2, i);
        arrayList.add(dkg);
        dkd dkd = new dkd(djc2, "WiMYovSpRj6RF1r6zXON0hYVyLxgllUAYyMyyeBevdbJbdYcoBpd8nDjCwwclcSu", "vpq7z91A+vN3MIS65A+GwiFRgEFOFpQOdK5Z+mjW21w=", dci2, i);
        arrayList.add(dkd);
        return arrayList;
    }

    public final void a(View view) {
        if (((Boolean) dqe.f().a(dtg.bE)).booleanValue()) {
            djm djm = this.y;
            if (djm == null) {
                this.y = new djm(a, view);
            } else {
                djm.a(view);
            }
        }
    }

    /* access modifiers changed from: protected */
    public final dci b(Context context) {
        dci dci = new dci();
        if (!TextUtils.isEmpty(this.v)) {
            dci.b = this.v;
        }
        djc b = b(context, this.r);
        if (b.b != null) {
            a(a(b, dci, (czk) null));
        }
        return dci;
    }

    /* access modifiers changed from: protected */
    public final djj b(MotionEvent motionEvent) {
        Method a = a.a("oT49Nu6m3oNdAQwbQzvwr/RE4TwZbR8mJKypakrI/GqBgyD2niyVic8N9UaqvU10", "LS3Cw7DlWSqpRsXrajoVsCxXOedCBnQXFZcLjXzD8so=");
        if (a == null || motionEvent == null) {
            throw new zzcw();
        }
        try {
            return new djj((String) a.invoke(null, new Object[]{motionEvent, this.q}));
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new zzcw(e);
        }
    }
}
