package defpackage;

/* renamed from: vgu reason: default package */
final class vgu extends vhe {
    private final boolean a;

    /* renamed from: vgu$a */
    static final class a extends defpackage.vhe.a {
        private Boolean a;

        a() {
        }

        public final defpackage.vhe.a a(boolean z) {
            this.a = Boolean.valueOf(z);
            return this;
        }

        public final vhe a() {
            String str = "";
            if (this.a == null) {
                StringBuilder sb = new StringBuilder();
                sb.append(str);
                sb.append(" mercuryPremiumHubsPageBackendV3Android");
                str = sb.toString();
            }
            if (str.isEmpty()) {
                return new vgu(this.a.booleanValue(), 0);
            }
            StringBuilder sb2 = new StringBuilder("Missing required properties:");
            sb2.append(str);
            throw new IllegalStateException(sb2.toString());
        }
    }

    /* synthetic */ vgu(boolean z, byte b) {
        this(z);
    }

    private vgu(boolean z) {
        this.a = z;
    }

    public final boolean a() {
        return this.a;
    }

    public final String toString() {
        StringBuilder sb = new StringBuilder("PremiumDestinationProperties{mercuryPremiumHubsPageBackendV3Android=");
        sb.append(this.a);
        sb.append("}");
        return sb.toString();
    }

    public final boolean equals(Object obj) {
        if (obj == this) {
            return true;
        }
        if (obj instanceof vhe) {
            if (this.a == ((vhe) obj).a()) {
                return true;
            }
        }
        return false;
    }

    public final int hashCode() {
        return (this.a ? 1231 : 1237) ^ 1000003;
    }
}
