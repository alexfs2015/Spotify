...
public class SessionClientImpl implements SessionClient {
    ...
    static class LoginResponseFunction implements Function<TypedResponse<LoginResponseBody>, LoginResponse> {
        ...
        public LoginResponse apply(TypedResponse<LoginResponseBody> typedResponse) {
            ...
            if (status >= 400) {
                return LoginResponse.error(SessionClientImpl.parseErrorCode((String) typedResponse.headers().get(LogMessage.SEVERITY_ERROR), status), (String) typedResponse.headers().get("message"));
            }
            ...
            if (!(loginResponseBody instanceof CodeRequired)) {
                return loginResponseBody instanceof BootstrapRequired ? LoginResponse.bootstrapRequired(((BootstrapRequired) loginResponseBody).accessToken()) : LoginResponse.error(500, "unsupported_response_type");
            }
