...
public final class BannerConfigurationRequester {
    ...
    public BannerConfigurationRequester(Context context, ObjectMapper objectMapper, RxResolver rxResolver, jrp jrp) {
        this.c = context;
        this.b = objectMapper;
        this.a = rxResolver;
        this.d = jrp;
    ...
    public /* synthetic */ BannerConfiguration b(Response response) {
        Logger.b("response status: %s, responseBody: %s", Integer.valueOf(response.getStatus()), new String(response.getBody(), far.c));
    ...
    private BannerConfiguration a(Response response) {
        try {
            ...
            throw new AssertionError(String.format(Locale.US, "Error parsing response %s for class %s", new Object[]{new String(response.getBody(), far.c), BannerConfiguration.class.getSimpleName()}));
        }
    }
}
