...
public class BeanSerializerFactory extends BasicSerializerFactory implements Serializable {
    ...
    public JsonSerializer<Object> constructBeanSerializer(SerializerProvider serializerProvider, BeanDescription beanDescription) {
        ...
        List arrayList = findBeanProperties == null ? new ArrayList() : removeOverlappingTypeIds(serializerProvider, beanDescription, constructBeanSerializerBuilder, findBeanProperties);
        ...
        if (this._factoryConfig.hasSerializerModifiers()) {
            for (BeanSerializerModifier changeProperties : this._factoryConfig.serializerModifiers()) {
                arrayList = changeProperties.changeProperties(config, beanDescription, arrayList);
            }
        }
        List filterBeanProperties = filterBeanProperties(config, beanDescription, arrayList);
        if (this._factoryConfig.hasSerializerModifiers()) {
        ...
        try {
            jsonSerializer2 = constructBeanSerializerBuilder.build();
        ...
        return (jsonSerializer2 != null || !beanDescription.hasKnownClassAnnotations()) ? jsonSerializer2 : constructBeanSerializerBuilder.createDummy();
    }
