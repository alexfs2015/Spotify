...
public class GuavaSerializers extends Base implements Serializable {
    ...
    public JsonSerializer<?> findMapLikeSerializer(SerializationConfig serializationConfig, MapLikeType mapLikeType, BeanDescription beanDescription, JsonSerializer<Object> jsonSerializer, TypeSerializer typeSerializer, JsonSerializer<Object> jsonSerializer2) {
        Set set = null;
        if (!fdf.class.isAssignableFrom(mapLikeType.getRawClass())) {
            return null;
        }
        ...
        Value defaultPropertyIgnorals = serializationConfig.getDefaultPropertyIgnorals(fdf.class, beanDescription.getClassInfo());
    ...
    public JsonSerializer<?> findSerializer(SerializationConfig serializationConfig, JavaType javaType, BeanDescription beanDescription) {
        ...
        if (fdv.class.isAssignableFrom(rawClass)) {
            return new TableSerializer(_findDeclared(javaType, fdv.class));
        }
        if (HostAndPort.class.isAssignableFrom(rawClass)) {
            return ToStringSerializer.instance;
        }
        if (feq.class.isAssignableFrom(rawClass)) {
            return ToStringSerializer.instance;
        }
        if (fbz.class.isAssignableFrom(rawClass) || fby.class.isAssignableFrom(rawClass)) {
            return ToStringSerializer.instance;
        }
        if (HashCode.class.isAssignableFrom(rawClass)) {
            return ToStringSerializer.instance;
        }
        if (!fcp.class.isAssignableFrom(rawClass)) {
