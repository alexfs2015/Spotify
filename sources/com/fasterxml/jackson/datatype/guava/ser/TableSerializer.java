...
public class TableSerializer extends ContainerSerializer<fdd<?, ?, ?>> implements ContextualSerializer {
    ...
    public boolean isEmpty(SerializerProvider serializerProvider, fdd<?, ?, ?> fdd) {
        return fdd.a();
    }

    public boolean hasSingleElement(fdd<?, ?, ?> fdd) {
        return fdd.b() == 1;
    }

    public void serialize(fdd<?, ?, ?> fdd, JsonGenerator jsonGenerator, SerializerProvider serializerProvider) {
        jsonGenerator.writeStartObject(fdd);
        if (!fdd.a()) {
            serializeFields(fdd, jsonGenerator, serializerProvider);
        }
        jsonGenerator.writeEndObject();
    }

    public void serializeWithType(fdd<?, ?, ?> fdd, JsonGenerator jsonGenerator, SerializerProvider serializerProvider, TypeSerializer typeSerializer) {
        jsonGenerator.setCurrentValue(fdd);
        WritableTypeId writeTypePrefix = typeSerializer.writeTypePrefix(jsonGenerator, typeSerializer.typeId(fdd, JsonToken.START_OBJECT));
        serializeFields(fdd, jsonGenerator, serializerProvider);
        typeSerializer.writeTypeSuffix(jsonGenerator, writeTypePrefix);
    }

    private final void serializeFields(fdd<?, ?, ?> fdd, JsonGenerator jsonGenerator, SerializerProvider serializerProvider) {
        this._rowMapSerializer.serializeFields(fdd.c(), jsonGenerator, serializerProvider);
    }
}
