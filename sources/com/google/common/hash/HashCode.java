...
public abstract class HashCode {
    ...
    static final class BytesHashCode extends HashCode implements Serializable {
        ...
        public final int b() {
            fay.b(this.bytes.length >= 4, "HashCode#asInt() requires >= 4 bytes (it only has %s bytes).", this.bytes.length);
            byte[] bArr = this.bytes;
            return ((bArr[3] & 255) << 24) | ((bArr[1] & 255) << 8) | (bArr[0] & 255) | ((bArr[2] & 255) << 16);
        }

        public final long c() {
            fay.b(this.bytes.length >= 8, "HashCode#asLong() requires >= 8 bytes (it only has %s bytes).", this.bytes.length);
    ...
    public static HashCode a(String str) {
        boolean z = true;
        fay.a(str.length() >= 2, "input string (%s) must have at least 2 characters", (Object) str);
        if (str.length() % 2 != 0) {
            z = false;
        }
        fay.a(z, "input string (%s) must have an even number of characters", (Object) str);
