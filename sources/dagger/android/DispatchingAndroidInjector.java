...
public final class DispatchingAndroidInjector<T> implements whq<T> {
    private final Map<String, wzi<a<?>>> a;
    ...
    public DispatchingAndroidInjector(Map<Class<?>, wzi<a<?>>> map, Map<String, wzi<a<?>>> map2) {
        this.a = a(map, map2);
    }

    private static <C, V> Map<String, wzi<a<?>>> a(Map<Class<? extends C>, V> map, Map<String, V> map2) {
        if (map.isEmpty()) {
            return map2;
        }
        LinkedHashMap b = wid.b(map.size() + map2.size());
        b.putAll(map2);
    ...
    private boolean b(T t) {
        wzi wzi = (wzi) this.a.get(t.getClass().getName());
        if (wzi == null) {
            return false;
        }
        a aVar = (a) wzi.get();
        try {
            ((whq) wil.a(aVar.a(t), "%s.create(I) should not return null.", aVar.getClass())).a(t);
            return true;
